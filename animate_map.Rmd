---
title: "R Notebook"
output: html_notebook
---

```{r}
library(readr)
library(tidyverse)
```
```{r}
require(devtools)
install_github('ramnathv/rCharts@dev')
install_github('ramnathv/rMaps')
library(rMaps)
```


```{r}
DATA_certificate <- DATA %>% 
  filter(case_status=="certified") %>% 
  group_by(worksite_state_abb,year) %>% 
  summarise(num=n())
```

```{r}
DATA_new <-  DATA_certificate %>% 
  ungroup(worksite_state_abb) %>% 
  mutate(worksite_state_abb=str_to_upper(worksite_state_abb))
```

```{r}
d1 <- ichoropleth(num~worksite_state_abb,data=DATA_new,animate='year',legend=FALSE,geographyConfig=list(popupTemplate="#!function(geo, data) {
                                      return '<div class=\"hoverinfo\"><strong>'+
                                       data.worksite_state_abb+
                                       '<br>' + data.num+
                                       '</strong></div>';}!#"))
```



```{r}
d1$save('rMaps.html', cdn = TRUE)
```

