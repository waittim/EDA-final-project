...
h1b_amazon <- h1b_df %>% 
  ...
  ungroup(job_title) %>% 
  mutate(job_title=ifelse(str_detect(job_title,'software development engineer'), 'software development engineer', 
                   ifelse(str_detect(job_title,'technical program manager'), 'technical program manager',job_title)))%>% 
  group_by(job_title,year) %>% 
  summarise(n=n()) %>% 
  arrange(-n)
...
h1b_micro <- h1b_df %>% 
  ...
  ungroup(job_title) %>% 
  mutate(job_title=ifelse(str_detect(job_title,'software engineer|software development engineer'), 'software engineer', 
                   ifelse(str_detect(job_title,'program manager'), 'program manager',job_title))) %>% 
  group_by(job_title,year) %>% 
  summarise(n=n()) %>% 
  arrange(-n)
...
ggplot(h1b_uber, aes(x = year, y = n, group = job_title)) +
  geom_line(alpha = 0.3, 
            size = 0.2) +
  ...
  theme_light()+
  theme(plot.title = element_text(hjust = 0.5))+
   geom_line(data = subset(h1b_uber, job_title %in% con),aes(col=job_title),size=0.7)+
  theme(plot.title = element_text(size = 14, family = "Helvetica", face = "bold"),
              text = element_text(size = 12, family = "Helvetica"),
              axis.title = element_text(face="bold"),
              axis.text.x=element_text(size = 11))
